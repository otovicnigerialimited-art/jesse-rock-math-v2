...
// Replace newLevel
code = code.replace(/const newLevel = Math\.floor\(newXP \/ 1000\) \+ 1;\n\s*/g, '');
...
code = code.replace(/level: updatedStats\?\.level \?\? \(Math\.floor\(\(stats\.xp \+ 150\) \/ 1000\) \+ 1\),/g, 'level: updatedStats?.level ?? 1,');
